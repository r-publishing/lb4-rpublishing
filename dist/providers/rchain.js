...
const prepareDeploy = async (httpUrlReadOnly, publicKey, timestamp) => {
    ...
    try {
        prepareDeployResponse = await rchainToolkit.http.prepareDeploy(httpUrlReadOnly, {
            deployer: publicKey,
            timestamp: timestamp,
            nameQty: 1,
        });
...
const validAfterBlockNumber = async (httpUrlReadOnly) => {
    ...
    try {
        validAfterBlockNumberResponse = JSON.parse(await rchainToolkit.http.blocks(httpUrlReadOnly, {
            position: 1,
        }))[0].blockNumber;
    }
    catch (err) {
 ...
 */
const transferFundsTerm = (revAddrFrom, revAddrTo, amount) => `
   ...
   `;
const fundWallet = async (revAddrFrom, revAddrTo, amount) => {
    const timestamp = new Date().getTime();
    const vab = await validAfterBlockNumber(READ_ONLY_HOST);
    const pd = await prepareDeploy(READ_ONLY_HOST, escrowPubKey, timestamp);
    const term = transferFundsTerm(revAddrFrom, revAddrTo, amount);
    const deployOptions = await rchainToolkit.utils.getDeployOptions('secp256k1', timestamp, term, escrowPrivKey, escrowPubKey, 1, 10000000, vab || -1);
    try {
        const deployResponse = await rchainToolkit.http.deploy(VALIDATOR_HOST, deployOptions);
...
const deployMaster = async (publisherPrivKey) => {
    var _a, _b;
    const depth = (_a = getDepth()) !== null && _a !== void 0 ? _a : 3;
    const contractDepth = (_b = getContractDepth()) !== null && _b !== void 0 ? _b : 2;
    const publicKey = rchainToolkit.utils.publicKeyFromPrivateKey(publisherPrivKey);
    const timestamp = new Date().getTime();
    const vab = await validAfterBlockNumber(READ_ONLY_HOST);
    const pd = await prepareDeploy(READ_ONLY_HOST, publicKey, timestamp);
    ...
    log('✓ prepare deploy');
    const deployOptions = await rchainToolkit.utils.getDeployOptions('secp256k1', timestamp, term, publisherPrivKey, publicKey, 1, 10000000, vab || -1);
    try {
        const deployResponse = await rchainToolkit.http.deploy(VALIDATOR_HOST, deployOptions);
        if (!deployResponse.startsWith('"Success!')) {
            log('Unable to deploy');
...
const deployBox = async (privateKey, publicKey, masterRegistryUri, boxId) => {
    const term = deployBoxTerm({
        publicKey: publicKey,
        boxId: boxId,
        masterRegistryUri: masterRegistryUri,
    });
    console.log('  02 deploy box is ' + Buffer.from(term).length / 1000000 + 'mb');
    const timestamp = new Date().getTime();
    const vab = await validAfterBlockNumber(READ_ONLY_HOST);
    const pd = await prepareDeploy(READ_ONLY_HOST, publicKey, timestamp);
    const deployOptions = await rchainToolkit.utils.getDeployOptions('secp256k1', timestamp, term, privateKey, publicKey, 1, 1000000, vab || -1);
    try {
        const deployResponse = await rchainToolkit.http.deploy(VALIDATOR_HOST, deployOptions);
        if (!deployResponse.startsWith('"Success!')) {
            console.log(deployResponse);
...
const deployContract = async (privateKey, masterRegistryUri, fungible, contractId, boxId, expires) => {
    console.log(`Will deploy a\x1b[36m`, fungible ? 'fungible' : 'non-fungible', '\x1b[0mtokens contract');
    ...
    //   .replace('/*DEFAULT_BAGS_DATA*/', defaultBagsDataRholang);
    log('✓ prepare deploy');
    const deployOptions = await rchainToolkit.utils.getDeployOptions('secp256k1', timestamp, term, privateKey, publicKey, 1, 10000000, vab || -1);
    try {
        const deployResponse = await rchainToolkit.http.deploy(VALIDATOR_HOST, deployOptions);
        if (!deployResponse.startsWith('"Success!')) {
            log('Unable to deploy');
...
const createPurse = async (privateKey, masterRegistryUri, contractId, boxId, type, newId, quantity, price) => {
    ...
    const deployOptions = await rchainToolkit.utils.getDeployOptions('secp256k1', timestamp, term, privateKey, publicKey, 1, 100000000, vab);
    try {
        const deployResponse = await rchainToolkit.http.deploy(VALIDATOR_HOST, deployOptions);
        if (!deployResponse.startsWith('"Success!')) {
            log('Unable to deploy');
...
const checkPursesInBox = async (masterRegistryUri, boxId, contractId, ids) => {
    ...
    if (allData.purses[contractId].filter((bid) => !!ids.find((id) => id === bid))
        .length !== ids.length) {
...
//# sourceMappingURL=rchain.js.map